...
void Test(int* a)
{
	...
	*pInt = 300;
	...
	// cpu가 중간연산결과 저장으로 빠르게 끌어다 쓰는 레지스터메모리가 있다.
	// 원래 cint에 있는 값인 100을 기억하고 문법상 상수이니 바꿀수 없다고 판단하여 100이 출력된 것

	// volatile const int cint = 100; 으로 쓰면 300이 나온다.
	// volatile은 휘발성이라는 뜻인데 레지스터를 이용하느 방식을 쓰지 말라는 것. 값을 제대로 확인해라?
